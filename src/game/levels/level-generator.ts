...
export function createLevelDefinition(id: number): LevelDefinition {
  ...
  const targetScore = buildTargetScore(
    config,
    rows,
    cols,
    fruitTypes,
  ...
  const star1 = roundToNearest50(targetScore * 0.68);
  const star2 = roundToNearest50(targetScore * 0.88);
  const star3 = roundToNearest50(targetScore * 1.12);
